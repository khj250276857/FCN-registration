...
def gen_loss_graph_from_log():
    workspace = r"C:\Users\khj\Desktop\running data\log"
    ...
    train_y_list = [float(_.split(',')[1].split('=')[-1].split('...')[0])/10 for _ in train_y_text]
    valid_y_list = [float(_.split(',')[1].split('=')[-1].split('...')[0])/10 for _ in valid_y_text]
